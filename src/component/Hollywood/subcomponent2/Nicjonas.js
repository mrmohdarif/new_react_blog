...
function Nicjonas() {
  ...
  return (
    ...
    {dataE.filter((item)=>item.categorey==="The Latest Article" && item.name==="Hollywood" && item.path==='/nicjonas').map((data,index)=>{
      return(
          <div className="papaya_sub"> 
              ...
              <img src={data.urlToImage} alt="the_latest_Javan" className='holy_article_img' style={{}} />
               ...
               <button className='backbtn_Nic' onClick={()=>{back(-1)}}>Back</button>
          </div>
      )
    })}
    </div>
