...
function Netflex() {
  ...
  return (
    ...
    {dataD.filter((item)=>item.categorey==="The Latest" && item.name==="Hollywood" && item.path==='/netflex').map((data,index)=>{
      return(
          <div className="papaya_sub"> 
              ...
              <img src={data.urlToImage} alt="the_latest_Javan"  className='Holy_img' style={{}} />
               ...
               <button className='backbtn_netflex' onClick={()=>{back(-1)}}>Back</button>
          </div>
      )
    })}
    </div>
