...
function Phulahari() {
    ...
    return (
      ...
      {dataE.filter((item)=>item.categorey==="The Latest" && item.name==="Food" && item.path==='/phulahari').map((data,index)=>{
        return(
            <div className="papaya_sub"> 
                ...
                <img src={data.urlToImage} alt="the_latest_Javan" className='Food_latest' style={{}} />
                 ...
                 <button className='backbtn_phula' onClick={()=>{back(-1)}}>Back</button>
            </div>
        )
      })}
      </div>
