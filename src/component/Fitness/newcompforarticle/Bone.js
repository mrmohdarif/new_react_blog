...
function Bone() {
    ...
    return (
      ...
      {dataE.filter((item)=>item.categorey==="The Latest Article" && item.name==="Fitness" && item.path==='/bone').map((data,index)=>{
        return(
            <div className="papaya_sub"> 
                ...
                <img src={data.urlToImage} alt="the_latest_Javan" className='fitness_article.img' style={{}} />
                 ...
                 <button className='backbtn_bone' onClick={()=>{back(-1)}}>Back</button>
            </div>
        )
      })}
      </div>
